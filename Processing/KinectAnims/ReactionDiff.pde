class ReactionDiff implements Drawable {
  int w=width;
  int h=height;
  ...
  float diffusionRateV = .02; //the activator


  float deltaT = 6; // time step
  ...
  ReactionDiff() {
    ...
    for (int i=0; i<w; ++i) {
      for (int j=0; j<h; ++j) {
        gridU[i][j] = .2+random(-.01, .01);
        gridV[i][j] = .1+random(-.01, .01);
      }
    }
  ...
  void setupDiffRates() {
    ...
    for (int i=0; i<w; ++i) {
      for (int j=0; j<h; ++j) {
        diffRateUYarr[i][j] = .01+(j)*.008/h;
        diffRateUXarr[i][j] = .03+i*.02/w;
